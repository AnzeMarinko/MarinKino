...
group_folders = {
    g: g[3:].replace("-", " ").replace("series", "(Serija)").title().strip()
    + f" ({groups.count(g)})"
...
def rate_movie():
    try:
        ...
        log.info(
            f"New ratings added to {movie_folder}. New ratings: {summary}"
        )
        ...
        try:
            ...
            log.info(
                "Obvestilo o novih ocenah poslan"
                f" administratorju za film {movie_folder}"
            )
...
def remcommend_movie():
    ...
    all_films[recommending_folder]["recommendation_level"] = (
    ...
    with open(recommending_metadata_file, "w", encoding="utf-8") as f:
        json.dump(movie_metadata, f, ensure_ascii=False, indent=4)
    log.info(
        f"Filmu {recommending_folder} nastavljen nivo priporočila:"
...
@movies_bp.route("/movies/file/<path:filename>")
@login_required
def movie_file(filename):
    try:
        # Preverimo varnost poti (da ne more nihče z ../ uiti iz mape s filmi)
        _ = safe_path(
            "../" + FILMS_ROOT,
            filename,
        )
        if not os.path.exists(os.path.join(FILMS_ROOT, filename)):
            abort(404)
    except ValueError:
        abort(404)

    if FLASK_ENV == "production":
        # Nginx X-Accel-Redirect (Produkcija)
        response = make_response()
        safe_uri_path = quote(filename)
        if not safe_uri_path.startswith("/"):
            safe_uri_path = "/" + safe_uri_path

        response.headers["X-Accel-Redirect"] = (
            f"/protected_movies{safe_uri_path}"
        )

        # Nginx bo sam določil MIME tip na podlagi končnic (.m3u8, .mp4, .vtt)
        response.headers["Content-Type"] = ""
    else:
        # Lokalni razvoj (Flask)
        mimetype = None
        ...
        # send_from_directory zdaj sprejme celotno pot
        # 'filename' neposredno iz korena
            ...
            if match:
                ...
                if start == 0:
                    try:
                        ...
                        data = redis_client.hget(user_key, aux_filename)
                        ...
                        redis_client.hset(
                            user_key, aux_filename, json.dumps(data)
                        )
                    except Exception as e:
    ...
    # Podnapisi (Lokalni razvoj)
    elif filename.endswith(".vtt") and FLASK_ENV != "production":
        response.headers["Content-Type"] = "text/vtt"
    elif filename.endswith(".srt") and FLASK_ENV != "production":
...
def add_comment():
    ...
    try:
        ...
        try:
            ...
            log.info(
                "Mail s komentarjem poslan administratorju"
                f" za film {movie_folder}"
            )
        except Exception as e:
...
def admin_comment():
    ...
    try:
        ...
        # Dodaj admin odgovor
        movie_metadata["user_notes"][comment_index]["admin_response"] = (
            admin_response
        )
...
def add_warning():
    ...
    try:
        ...
        log.info(
            f"Admin opozorilo '{warning_type}' dodano na film"
            f" {movie_folder} od {current_user.id}"
        )
        return {
            "status": "success",
...
def delete_warning():
    ...
    try:
        ...
        log.info(
            f"Opozorilo '{deleted_note.get('text')}'"
            f" izbrisano z {movie_folder}"
        )
        return {
            "status": "success",
