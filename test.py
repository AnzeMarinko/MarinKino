...
all_films = [{"old": movie_to_dict(m), "new": movie_to_dict(m_new)} for m, m_new in zip(check_folder(FILMS_ROOT, use_new=False)[::-1], check_folder(FILMS_ROOT, use_new=True)[::-1])]
...
changing_names = ["Title", "OriginalTitle", "Year", "Plot", "Genres", "Players"]
...
def change():
    for idx in list(DECISIONS.keys()):
                ...
                if k == "cover":
                    ...
                    if "logo.png" not in new_file:
                        if "logo.png" not in old_file:
                            ...
                            os.rename(new_file, new_file.replace("new_cover", "cover"))
                else:
        ...
        new_movie_metadata["RuntimesByFiles"] = movie_metadata.get("RuntimesByFiles", {})

        with open(film_readme_file, 'w', encoding="utf-8") as f:
            json.dump(new_movie_metadata, f, ensure_ascii=False, indent=4)

        if os.path.exists(os.path.join(new_metadata["folder"], "new_cover_thumb.jpg")):
            os.remove(os.path.join(new_metadata["folder"], "new_cover_thumb.jpg"))
        if os.path.exists(os.path.join(new_metadata["folder"], "new_readme.json")):
